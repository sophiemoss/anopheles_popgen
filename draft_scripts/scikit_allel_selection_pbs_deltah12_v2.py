...
all_seg_variants = ac_all.is_segregating() & ac_all.is_biallelic_01()
...
# %% Plot H12 values against genomic position

import matplotlib.pyplot as plt
import numpy as np

# Assuming resh12 and sush12 are your H12 values for resistant and susceptible samples respectively
# and pos_res_seg and pos_sus_seg are the positions of the variants in these groups

# Calculate midpoints of each 1000 bp window
window_size = 1000
num_windows_res = len(pos_res_seg) // window_size
num_windows_sus = len(pos_sus_seg) // window_size
midpoints_res = np.arange(window_size / 2, num_windows_res * window_size, window_size)
midpoints_sus = np.arange(window_size / 2, num_windows_sus * window_size, window_size)

# Ensure that the length of resh12, sush12 and midpoints are the same
# Trim the H12 arrays if they are longer than the midpoints arrays
resh12 = resh12[:len(midpoints_res)]
sush12 = sush12[:len(midpoints_sus)]
