    ...
    for window_percentile_values in percentile_99th_values:
        ...
        if np.array_equal(window, percentile_window):
            ...
            if real_fst_value > percentile_99th:
                ...
                significant_data.append({'Window': window_str, 'Fst Value': real_fst_value, '99th Percentile from permutations': percentile_99th})

# Create DataFrame from the list
significant_values_df = pd.DataFrame(significant_data)
# save to a csv
significant_values_df.to_csv(f'significant_fst_values_{pop1}_{pop2}_{chromosome}.csv', index=False)

# %% Remove noise using the threshold set using the histogram earlier
# Read the CSV file into a DataFrame
significant_values_df = pd.read_csv(f'significant_fst_values_{pop1}_{pop2}_{chromosome}.csv')
print(f"Read in significant fst values from csv")
...
print(f"Filtered for Fst values that are significant AND are above the histogram threshold")
print(filtered_df.head())

# %% Save the filtered DataFrame to a CSV file
filtered_df.to_csv(f'significant_filtered_fst_values_{pop1}_{pop2}_{chromosome}.csv', index=False)

# %% Plot these filtered points on the combined fst graph by highlighting significant points
# Read in the filtered significant values
filtered_df = pd.read_csv(f'significant_filtered_fst_values_{pop1}_{pop2}_{chromosome}.csv')
print("Read in filtered significant fst values from csv")

# %% Extract the midpoint of each window for the x-coordinate
filtered_df['Window Midpoint'] = filtered_df['Window'].apply(lambda w: np.mean([int(x) for x in w.split('-')]))

# %% Plotting the combined FST graph
fig, ax = plt.subplots(figsize=(10, 4))
sns.despine(ax=ax, offset=5)
...
ax.scatter(filtered_df['Window Midpoint'], filtered_df['Fst Value'], color='blue', s=10, label='Significant FST')
