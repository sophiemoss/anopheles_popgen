...
while read -r bamfile; do
    ...
    filteredsortedbam="${output%.*}.filtered.bam"
    samtools view -b -q 10 "$output" > "$filteredsortedbam"
    echo "Processed $output ->$filteredsortedbam by filtering the sorted bamfile"
