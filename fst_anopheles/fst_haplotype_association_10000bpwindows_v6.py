...
# %% First, define the dendrogram function: (added the creation of the window_linkage_matrices dictionary to this function)
...
def dendrogram(
    ...
    window_linkage_matrices[window] = linkage_matrix
...
for window, linkage_matrix in window_linkage_matrices.items():
    ...
    clusters_with_more_than_20 = {k: v for k, v in clustered_haplotypes.items() if len(v) >= 20}

    # Only add to the dictionary if there are clusters with more than five haplotypes
    if clusters_with_more_than_20:
        window_clusters_with_many_haplotypes[window] = clusters_with_more_than_20
        print(f"Clusters for window {window} with many haplotypes:")
        for cluster_label, haplotype_indices in clusters_with_more_than_20.items():
